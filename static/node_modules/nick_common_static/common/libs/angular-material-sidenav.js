                ...
                var onStateChangeStart = function (event, toState, toParams) {
                    ...
                    function digest(sections, currentSection) {
                        sections.forEach(function (section) {
                            if (section.children) {
                                digest(section.children, section);
                            } else if (section.pages) {
                                digest(section.pages, section);
                            } else if (section.type === 'link') {
                                return !matchPage(currentSection, section, newState);
                            }
                        });
...
//                    });
                ...
                self = {
                    ...
                    isSectionSelected: function (section) {
                        ...
                        return self.openedSection.id === section.id;
                    },
...
//                onStateChangeStart(null, $state.current, $stateParams);
                            ...
                            _apply_color = function () {
                                for (var p in $scope.ssStyleColor) {
                                    if ($scope.ssStyleColor.hasOwnProperty(p)) {
                                        themeColors = ssSideNavSections.theme.colors,
        ...
        .run(['$templateCache', function ($templateCache) {
            $templateCache.put('views/ss/menu-link.tmpl.html',
                '<md-button\n' +
                '   ss-style-color="{\'background-color\': isSelected(section.state) ? \'warn.300\': \'primary.default\'}"' +
                '   class="md-raised md-primary"' +
                '   ui-sref="{{section.state}}"\n' +
                '   ng-click="focusSection(section)">\n' +
                '   <i class="fa" ng-class="section.icon"></i>' +
                '   {{section.name}}\n' +
            ...
            $templateCache.put('views/ss/menu-toggle.tmpl.html',
                ...
                '   <div flex layout="row">\n' +
                '       <span><i class="fa" ng-class="section.icon"></i>{{section.name}}</span>' +
                '       \n' +
                '       <span flex></span>\n' +
                ...
                '</md-button>\n' +
                '\n' +
                '<md-divider ng-if="isOpen(section)"></md-divider>' +
